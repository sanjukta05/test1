...
const TextureSample: React.FC<TextureSampleProps> = ({ name, textureClass, description }) => {
  return (
...
export const ArchAccent: React.FC<{ className?: string }> = ({ className = "" }) => {
  return (
...
export const GoldAccent: React.FC<{ width?: string; className?: string }> = ({ width = "w-24", className = "" }) => {
  return (
    <div className={`h-0.5 bg-deco-gold mx-auto ${width} ${className}`}></div>
  );
};

export default { TextureGrid, ArchAccent, GeometricDivider, GoldAccent };
