...
const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return <section className="relative py-20 md:py-32 bg-deco-plum overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className={`max-w-4xl ${centered ? 'mx-auto text-center' : 'ml-0 text-left'}`}>
          ...
          {description && <p className="font-circular text-xl md:text-xl text-zinc-50">{description.endsWith('.') ? description : description + '.'}</p>}
        </div>
      </div>
      
      {/* Dubai Skyline Silhouette */}
      <div className="absolute bottom-0 left-0 right-0 h-16 bg-contain bg-repeat-x bg-bottom opacity-20" style={{
      backgroundImage: 'url(/images/dubai-skyline.svg)'
    }}></div>
    </section>;
};
